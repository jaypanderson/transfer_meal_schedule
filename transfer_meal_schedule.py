...
def extract_meal_data_small_kids(path: str) -> Union[dict, None]:
    ...
    user is prompted to choose a file and closes the window without choosing. This returns a '' empty string
    """
    if path == '':
        return None
    book = openpyxl.load_workbook(path)
...
def paste_meal_data(path: str, meal_data_big_kids: dict, meal_data_small_kids: dict) -> None:
    ...
    for key, val_big in meal_data_big_kids.items():
        ...
        # checks to see if meal data is None and if the date exists in its dictionary. this can happen if the excell
